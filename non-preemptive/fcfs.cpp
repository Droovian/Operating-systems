...
class fcfs{
    ...
    void calculate_tat(int n, fcfs *p){
    ...
    void calculate_wt(int n, fcfs *p){
        for (int i = 0; i < n; i++){
            p[i].waiting_time= p[i].tat - p[i].burst_time;
        }
    }

    void calculate_avg(int n, fcfs *process, float &avg_wt, float &avg_tat){
    ...
    bool compareValue(fcfs & a,fcfs & b){
            
            return a.arrival_time < b.arrival_time;
        }

    void fcfs::get_data(int n){
